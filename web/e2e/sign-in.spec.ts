...
test("focuses the verification state after sending a code", async ({ page }) => {
  ...
  await expect(page.locator('input[placeholder="123456"]:visible')).toBeFocused();
...
test("expires codes and keeps resend on cooldown", async ({ page }) => {
  ...
  await expect(page.getByRole("alert").filter({ hasText: /code has expired/i })).toBeVisible({
    timeout: 3_000,
  });
...
test("presents actionable OTP and rate-limit errors", async ({ page }) => {
  ...
  await expect(page.getByRole("alert")).toHaveText(/code is incorrect/i);
  ...
  await page.locator("[data-auth-email]:visible").press("Enter");
  await expect(page.getByRole("alert")).toHaveText(/wait 15 minutes/i);
});
...
test("treats Google cancellation as non-fatal and explains blocked popups", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /continue with google/i }).click();
  await expect(page.getByRole("alert")).toHaveText(/allow pop-ups/i);
});
